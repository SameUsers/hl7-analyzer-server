...
class AnalyzerHandler(HandlerInterface):
    ...
    Attributes:
        _builder: Билдер для создания структуры результата
        _buffer: Буфер для накопления входящих данных
        _parser: Парсер для разбора сообщений
        _framer: Фреймер для выделения сообщений из потока
    """
    ...
    def __init__(
        ...
        Args:
            builder: Билдер, создающий структуру результата из данных
            buffer: Буфер для временного хранения входящих данных
            parser: Парсер для обработки текстовых сообщений
            framer: Фреймер для выделения сообщений из потока
            storage: Хранилище для сохранения результатов (опционально)

        Raises:
            RuntimeError: Если не передан билдер
        """
    ...
    async def _on_data(self, chunk: bytes) -> AnalyzeResult | None:
        ...
        Returns:
            AnalyzeResult | None: Результат анализа или None,
